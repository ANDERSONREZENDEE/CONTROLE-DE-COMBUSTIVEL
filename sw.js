...
const ARQUIVOS_FIXOS = [
  ...
  './sw.js',
  './base_dados_wm.js',
  './base_dados_wm.js?v=39',
  './base_dados_2_nao_wm.js',
  './base_dados_2_nao_wm.js?v=39',
  './base_senhas.js',
  './base_senhas.js?v=39',
  './LOGOTIPO.jpg',
  './controladoria.jpg',
  ...
  './icone_principal_v14.png?v=39',
  './icone_atalho_v14.png',
  './icone_atalho_v14.png?v=39',
  './icone_combustível_v1.png',
  'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css',
  'https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js'
];

function ehArquivoAtualizavel(url) {
  return url.pathname.endsWith('/base_senhas.js') ||
         url.pathname.endsWith('/base_dados_wm.js') ||
         url.pathname.endsWith('/base_dados_2_nao_wm.js');
}
...
async function responderAtualizavel(evento) {
  const cache = await caches.open(CACHE_NAME);
  ...
  try {
    ...
    if (respostaRede && respostaRede.ok) {
      ...
      const urlSemQuery = new URL(evento.request.url);
      urlSemQuery.search = '';
      await cache.put(urlSemQuery.href, respostaRede.clone());
  ...
  return (
    await caches.match(evento.request, { ignoreSearch: true }) ||
    await caches.match('./base_senhas.js', { ignoreSearch: true }) ||
    new Response('', { status: 504, statusText: 'Offline e base não disponível no cache' })
  );
}
...
async function responderArquivo(evento) {
  ...
  if (ehArquivoAtualizavel(url)) {
    return responderAtualizavel(evento);
  }
